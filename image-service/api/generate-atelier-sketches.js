...
export default async function handler(req,res){
  ...
  try{
    ...
    const layout=components.map((x,i)=>({instanceId:x?.instanceId||`element-${i+1}`,catalogId:x?.catalogId||'',variantId:x?.variantId||x?.referenceId||'',name:x?.name||`element ${i+1}`,category:x?.category||x?.moodKind||'',moodKind:x?.moodKind||'',tags:Array.isArray(x?.tags)?x.tags:[],x:Number(x?.x||0),y:Number(x?.y||0),width:Number(x?.width||0),height:Number(x?.height||0),scale:Number(x?.scale||1),rotation:Number(x?.rotation??x?.rotate??0),zIndex:Number(x?.zIndex??i+5),order:Number(x?.order??i)}));
    ...
    noteTexts.forEach(text=>{if(String(text).trim())proximity.push(`Designer note on moodboard: ${String(text).trim().slice(0,180)}`)});
    ...
    const boardText=[componentNames.length&&`Selected references: ${componentNames.join(', ')}`,categories.length&&`Selected families: ${categories.join(', ')}`,materialNames.length&&`Selected materials: ${materialNames.join(', ')}`,colorNames.length&&`Selected colors: ${colorNames.join(', ')}`,patternNames.length&&`Selected patterns: ${patternNames.join(', ')}`,!patternNames.length&&`No pattern selected: do not invent a dominant pattern`,layoutText&&`Visual composition: ${layoutText}`,proximity.length&&`Spatial relationships and notes: ${proximity.join(' | ')}`,promptTokens.length&&`Technical vocabulary: ${[...new Set(promptTokens)].join(' | ')}`,designer.level&&`Designer technical level: ${designer.level}`].filter(Boolean).join('. ');
    ...
    async function classic(prompt,seed){const r=await fetch('https://api.magnific.com/v1/ai/text-to-image',{method:'POST',headers,body:JSON.stringify({prompt,negative_prompt:'photorealistic photo, 3d render, mannequin photo, runway photo, multiple outfits, multiple people, collage, text, watermark, logo, UI, moodboard, distorted anatomy, extra limbs, unrelated dominant garment feature, unselected dominant color, unselected dominant pattern',guidance_scale:2,seed,num_images:1,styling:{effects:{framing:'portrait'}},filter_nsfw:true})});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(`magnific_${r.status}:${JSON.stringify(j).slice(0,300)}`);const item=Array.isArray(j?.data)?j.data[0]:null,b64=item?.base64||null,url=item?.url||item?.generated?.[0]||null;if(!b64&&!url)throw new Error('magnific_image_missing');return url||`data:image/png;base64,${b64}`}
    ...
    return res.status(200).json({ok:true,provider:'magnific',count:3,proposals:results,meta:{components:componentNames,layout,relationships:proximity,materials:materialNames,colors:colorNames,patterns:patternNames,notes:noteTexts,moodboardReferenceUsed:!!capture,captureError:board?.capture?.error||null,generatedPromptUsed:!!generatedPrompt}});
