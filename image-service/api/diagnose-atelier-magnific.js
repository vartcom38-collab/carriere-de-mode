...
export default async function handler(req,res){
  ...
  try{
    ...
    return res.status(r.ok?200:502).json({ok:r.ok,stage:'classic-fast',keySource:process.env.MAGNIFIC_API_KEY?'MAGNIFIC_API_KEY':'MAGNIFIC_KEY',upstreamStatus:r.status,hasDataArray:Array.isArray(body?.data),dataLength:data.length,firstKeys:data[0]?Object.keys(data[0]).slice(0,20):[],hasBase64:!!data[0]?.base64,hasUrl:!!data[0]?.url,bodyPreview:r.ok?undefined:JSON.stringify(body).slice(0,1200)});
