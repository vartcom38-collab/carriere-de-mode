...
function boot(){
 ...
 function recoverTerritorialUnlocks(){let raw=read(KEY,[]);if(!Array.isArray(raw))raw=[];const seen=new Set(raw.map(x=>typeof x==='string'?x:x?.id).filter(Boolean));let changed=false;for(const [code,key] of Object.entries(TERRITORIAL_STORES)){const state=read(key,null);for(const signal of state?.signals||[]){if(signal?.kind!=='atelier_unlock'||!signal.payload)continue;const d=signal.payload,id='territory:'+code+':'+String(d.id||signal.id)+':atelier';if(seen.has(id))continue;raw.push({id,name:d.title||d.name||'Référence territoriale',label:d.title||d.name||'Référence territoriale',type:d.unlockType||d.type||'DESIGN_REFERENCE',territory:d.city||'',departmentCode:code,source:'territorial',materials:d.materials||[],motifs:d.motifs||[],palette:d.palette||[],learnedAt:new Date().toISOString(),level:d.level||'observed',description:d.text||d.description||'',meta:d.meta||{}});seen.add(id);changed=true}}
 ...
 function resolveTerritorialUnlocks(){const raw=read(KEY,[]);if(!Array.isArray(raw)||!raw.length)return[];const resolved=read(TERRITORIAL_RESOLUTIONS,{});const out=[];for(const ref of raw){if(!ref||typeof ref==='string'||ref.source!=='territorial'||!ref.id||resolved[ref.id])continue;const type=territorialPlaceType(ref);const items=unlockFrom('place',type,{count:1,sourceId:ref.id,city:ref.territory||null,placeId:ref.id,context:{territorial:true,departmentCode:ref.departmentCode||'',territorialReferenceId:ref.id,territorialReferenceName:ref.name||ref.label||'',materials:ref.materials||[],motifs:ref.motifs||[],palette:ref.palette||[]}});if(items.length){resolved[ref.id]={catalogIds:items.map(x=>x.id),resolvedAt:new Date().toISOString(),type};out.push({reference:ref,items})}}
 ...
 window.addEventListener('storage',e=>{if(e.key===KEY||Object.values(TERRITORIAL_STORES).includes(e.key))setTimeout(syncTerritorial,0)});
