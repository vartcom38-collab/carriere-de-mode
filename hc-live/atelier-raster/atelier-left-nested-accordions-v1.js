/* Haute Couture Live — nested creative accordions v1
   Catégorie -> sous-catégorie -> vignettes.
*/
(function(){
'use strict';
if(window.__HC_LEFT_NESTED_ACCORDIONS_V1__)return;
window.__HC_LEFT_NESTED_ACCORDIONS_V1__=true;
...
function render(){const host=$('#hcv2AccordionHost');if(!host||!C()||!window.HCAtelierBoardComponents){setTimeout(render,120);return}const all=items();const html=F.map((fam,fi)=>{const subs=fam.subs.map(([label,test])=>[label,all.filter(test)]).filter(x=>x[1].length);const total=subs.reduce((n,x)=>n+x[1].length,0);if(!total)return'';return `<details class="hcn-family" ${fi===0?'open':''}><summary><span>${esc(fam.label)}</span><i>${total}</i><em>⌄</em></summary><div class="hcn-subwrap">${subs.map(([label,list],si)=>`<details class="hcn-sub" ${fi===0&&si===0?'open':''}><summary><span>${esc(label)}</span><i>${list.length}</i><em>⌄</em></summary><div class="hcn-grid">${list.map(card).join('')}</div></details>`).join('')}</div></details>`}).join('');
 const fabrics=read(FABRICS,[]);const mat=fabrics.length?`<details class="hcn-family"><summary><span>MATIÈRES</span><i>${fabrics.length}</i><em>⌄</em></summary><div class="hcn-subwrap"><details class="hcn-sub"><summary><span>Tissus débloqués</span><i>${fabrics.length}</i><em>⌄</em></summary><div class="hcn-grid">${fabrics.map(fabricCard).join('')}</div></details></div></details>`:'';
 host.innerHTML=html+mat;bind(host);document.body.classList.add('hc-nested-accordion-ready')}
...
function boot(){css();render();window.addEventListener('storage',render);window.addEventListener('hc-atelier-components-changed',()=>{});}
