/* Haute Couture Live — garde-fou Projet actif v1 */
(function(){
'use strict';
if(window.__HC_ATELIER_PROJECT_GUARD_V1__)return;window.__HC_ATELIER_PROJECT_GUARD_V1__=true;
...
window.addEventListener('hc-atelier-project-changed',()=>{wrapPrompt();const p=active();if(!p||p.type!=='client')setTimeout(removeClientUi,0)});
window.addEventListener('hc-atelier-project-closed',()=>{setTimeout(removeClientUi,0);setTimeout(removeClientUi,250)});
let n=0;const t=setInterval(()=>{n++;wrapPrompt();if(ws()&&!active())removeClientUi();if(n>50)clearInterval(t)},120);
})();