/* Haute Couture Live — garde-fou Projet actif v3 */
(function(){
'use strict';
if(window.__HC_ATELIER_PROJECT_GUARD_V3__)return;window.__HC_ATELIER_PROJECT_GUARD_V3__=true;
...
function neutralBrief(){const p=active(),idea=String(p?.idea||'').trim();return{name:p?.name||'Création personnelle',garment:p?.name||'Création libre',occasion:p?.subtitle||'',style:'',paletteLiked:[],paletteAvoid:[],materialsPreferred:[],notes:[p?.subtitle||'',idea?('INTENTION CRÉATIVE : '+idea):''].filter(Boolean).join('\n'),creativeIdea:idea,budget:0,orderId:p?.id||''}}
...
window.addEventListener('hc-atelier-project-idea',()=>wrapPrompt());
