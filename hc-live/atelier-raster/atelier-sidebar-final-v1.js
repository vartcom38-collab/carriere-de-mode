/* Haute Couture Live — Atelier sidebar FINAL v1
   Menu robuste indépendant du shell : catégorie -> sous-catégorie -> vignettes.
*/
(function(){
'use strict';
if(window.__HC_ATELIER_SIDEBAR_FINAL_V1__) return;
window.__HC_ATELIER_SIDEBAR_FINAL_V1__=true;
...
function known(){const c=catalog();const starter=c?.starterIds instanceof Set?c.starterIds:new Set(c?.starterIds||[]);const raw=read(UNLOCK,[]);const unlocked=new Set((Array.isArray(raw)?raw:[]).map(v=>typeof v==='string'?v:v?.id).filter(Boolean));const set=new Set([...starter,...unlocked]);if(!set.size){for(const x of c?.items||[])if(Number(x.tier||99)<=1)set.add(x.id)}return set}
...
function card(x){const im=image(x);return `<button class="hcf-card" draggable="true" data-id="${esc(x.id)}"><span>${im?`<img src="${esc(im)}" alt="${esc(x.name)}">`:'✦'}</span><b>${esc(x.name)}</b></button>`}
