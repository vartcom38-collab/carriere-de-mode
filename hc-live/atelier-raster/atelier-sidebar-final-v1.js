/* Haute Couture Live — Atelier sidebar FINAL v1.1
...
(function(){
'use strict';
if(window.__HC_ATELIER_SIDEBAR_FINAL_V11__) return;
window.__HC_ATELIER_SIDEBAR_FINAL_V11__=true;
...
function render(){const host=ensureHost(),c=catalog();if(!host||!c){setTimeout(render,150);return}const all=items();let out='';for(const [family,subs] of TREE){const used=new Set();const built=subs.map(([label,test])=>{const list=all.filter(x=>test(x)&&!used.has(x.id));list.forEach(x=>used.add(x.id));return [label,list]}).filter(([,list])=>list.length);const total=built.reduce((n,[,list])=>n+list.length,0);if(!total)continue;out+=`<details class="hcf-family"><summary><span>${esc(family)}</span><i>${total}</i><em>⌄</em></summary><div class="hcf-subwrap">${built.map(([label,list])=>`<details class="hcf-sub"><summary><span>${esc(label)}</span><i>${list.length}</i><em>⌄</em></summary><div class="hcf-grid">${list.map(card).join('')}</div></details>`).join('')}</div></details>`}rendering=true;host.innerHTML=out;host.dataset.finalSidebar='1';bind(host);rendering=false}
