...
function collect(){
 ...
 const practice=read('haute-couture-career-practice-signals-v1',[]);
 for(const p of practice.slice(0,120)){
  const text=[p.axis,p.label,p.note,p.place].filter(Boolean).join(' ').toLowerCase();
  if(text)signals.push({tag:text,source:'practice',weight:Math.max(.5,Math.min(2,Number(p.strength||1)))})
 }
 return signals;
}
const THEMES=[
...
['hc-book-state','hc-client-order','hc-skill-unlock','hc-atelier-unlock','hc-nimes-social-message','hc-territorial-encounter','hc-career-practice-signal'].forEach(ev=>window.addEventListener(ev,()=>setTimeout(refresh,50)));
window.HCCareerEmergentIdentityV1={version:2,collect,compute,refresh,publicProfile,storageKey:KEY};
})();