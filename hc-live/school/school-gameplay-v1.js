...
const PENDING='haute-couture-school-pending-time-v1';
const DRAW='haute-couture-drawing-practice-v1';
...
function academic(){return window.HCSchoolAcademic||null}
function life(){return window.HCSchoolLife||null}
function course(id){return life()?.courses?.find(c=>c.id===id)||null}
function ensureHomework(courseId){const A=academic(),rule=homeworkRules[courseId];if(!A||!rule)return null;const s=A.state();if(s.assignments[rule.id])return s.assignments[rule.id];return A.assign({id:rule.id,title:rule.title,courseId,brief:rule.brief,dueDay:s.day+rule.dueOffset,kind:rule.kind})}
...
function consumeCourseTime(courseId){const c=course(courseId);if(!c)return;const key='course:'+courseId;const done=read(PENDING,{});if(done[key])return;done[key]={minutes:Number(c.duration)||0,at:new Date().toISOString()};write(PENDING,done);const run=()=>{if(window.HCGame?.advanceTime){window.HCGame.advanceTime(Number(c.duration)||0,'École — '+c.title);return true}return false};if(!run()){let tries=0;const t=setInterval(()=>{tries++;if(run()||tries>30)clearInterval(t)},150)}}
...
function dayChecklist(){const A=academic(),L=life();if(!A||!L)return null;const a=A.state(),l=L.state();const todays=L.courses.filter(c=>Number(c.day)===Number(a.day));const done=todays.filter(c=>l.completed[c.id]);const missing=todays.filter(c=>!l.completed[c.id]);const due=Object.values(a.assignments).filter(x=>x.dueDay<=a.day&&x.status==='assigned');return{courses:todays.length,done:done.length,missing,due,canClose:missing.length===0}}
