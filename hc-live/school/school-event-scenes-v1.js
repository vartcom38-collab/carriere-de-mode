...
const beats={
 conference:{intro:'La salle se remplit. L’intervenant parle autant de ses erreurs que de ses réussites. Tu n’as pas besoin de tout retenir : tu choisis ce que tu veux réellement creuser.',choices:[
  {id:'method',label:'Noter une méthode de travail',effect:'research',note:'Tu retiens une méthode ou une façon d’organiser le travail, sans transformer la conférence en savoir-faire acquis automatiquement.'},
  {id:'question',label:'Poser une question précise',effect:'confidence',note:'Tu formules une question liée à une difficulté concrète de ton parcours.'},
  {id:'observe',label:'Écouter sans intervenir',effect:'observe',note:'Tu écoutes et tries ce qui te semble utile sans te forcer à parler.'}
 ]},
 expo:{intro:'Les travaux sont visibles côte à côte. Ce qui frappe n’est pas seulement le niveau : ce sont les choix, les écarts et les manières différentes de résoudre un même problème.',choices:[
  {id:'compare',label:'Comparer deux démarches',effect:'research',note:'Tu compares deux manières de construire une réponse sans chercher un classement.'},
  {id:'talk',label:'Discuter avec un camarade',effect:'relation',note:'Tu demandes à un camarade ce qu’il a changé entre recherche et rendu final.'},
  {id:'alone',label:'Faire le tour seule',effect:'observe',note:'Tu prends le temps de regarder sans transformer l’exposition en networking obligatoire.'}
 ]},
 'render-night':{intro:'La fatigue est là, mais l’ambiance est concentrée. Certains parlent beaucoup, d’autres travaillent en silence. Tu dois choisir où placer ton temps avant le rendu.',choices:[
  {id:'help',label:'Aider quelqu’un sur un point précis',effect:'relation',note:'Tu aides sur un point concret sans prendre en charge le projet d’un autre.'},
  {id:'focus',label:'Protéger mon temps de travail',effect:'focus',note:'Tu restes concentrée sur ton propre rendu et poses une limite claire.'},
  {id:'review',label:'Faire une critique croisée de 20 min',effect:'research',note:'Tu échanges un retour rapide et demandes une critique sur ton propre travail.'}
 ]},
 atelier:{intro:'L’école reste ouverte plus longtemps. Les tables sont occupées, les prototypes sortent, les discussions deviennent plus directes à mesure que le rendu approche.',choices:[
  {id:'test',label:'Profiter du temps pour tester',effect:'research',note:'Tu utilises le moment pour vérifier une hypothèse au lieu de simplement avancer plus vite.'},
  {id:'peer',label:'Demander un avis ciblé',effect:'relation',note:'Tu sollicites un regard précis, pas une validation générale.'},
  {id:'work',label:'Travailler en silence',effect:'focus',note:'Tu avances sans transformer chaque soirée d’atelier en événement social.'}
 ]},
 social:{intro:'Ce moment existe surtout pour faire respirer la vie d’école. Les discussions ne débouchent pas forcément sur une opportunité : parfois elles servent simplement à mieux connaître la promo.',choices:[
  {id:'open',label:'Parler avec quelqu’un que je connais peu',effect:'relation',note:'Tu élargis doucement ton cercle sans obtenir de récompense automatique.'},
  {id:'close',label:'Rester avec mes proches',effect:'relation',note:'Tu renforces une relation déjà existante.'},
  {id:'leave',label:'Partir plus tôt',effect:'boundary',note:'Tu quittes le moment quand tu en as envie, sans sanction sociale automatique.'}
 ]}
};
...
function render(){
 /* Event scenes belong to the dedicated school-life screen. Never inject them into
    the guided home/day/course screens, especially during the first-day flow. */
 if(!/\/school-life\/?$/.test(path))return;
 const host=document.querySelector('#app')||document.querySelector('main')||document.body;if(!host||document.getElementById('hc-school-event-scenes'))return;const list=availableScenes().slice(0,3);if(!list.length)return;const root=document.createElement('section');root.id='hc-school-event-scenes';root.className='card';root.innerHTML=`<div class="hcse-eye">SCÈNES D’ÉCOLE</div><h2>Moments à vivre</h2><p class="hcse-lead">Les événements ne donnent pas automatiquement une compétence ou un contact. Tu choisis ce que tu fais réellement sur place, et le jeu garde la trace de ce que tu en retires.</p><div class="hcse-grid">${list.map(e=>{const sc=e.scene,b=beats[sc.type]||beats.social,people=sc.people.map(id=>students().find(p=>p.id===id)?.name).filter(Boolean);return `<article class="hcse-card"><div class="hcse-type">${sc.type.replace('-', ' ')}</div><h3>${e.title}</h3><p>${b.intro}</p>${people.length?`<div class="hcse-people">Présents : ${people.join(' · ')}</div>`:''}<div class="hcse-actions">${b.choices.map(c=>`<button data-scene="${e.id}" data-choice="${c.id}">${c.label}</button>`).join('')}</div></article>`}).join('')}</div>`;
