...
function loadPlaceDiscovery(){if(!/\/ville(?:\/|$)/i.test(location.pathname)||window.HCTerritorialPlaceDiscoveryV1||document.querySelector('script[data-hc-territorial-place-discovery]'))return;const marker='/hc-live/',p=location.pathname,i=p.indexOf(marker),base=i>=0?p.slice(0,i)+marker:'/';const s=document.createElement('script');s.src=base+'ville/territorial-place-discovery-v1.js?v=20260909-placediscovery1';s.defer=true;s.setAttribute('data-hc-territorial-place-discovery','1');document.head.appendChild(s)}
