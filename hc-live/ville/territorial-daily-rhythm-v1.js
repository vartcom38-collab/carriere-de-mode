...
function boot(){emit('boot');['hc-presence-changed','hc-residence-changed','hc-local-life-context-changed','hc-game-state'].forEach(ev=>window.addEventListener(ev,()=>setTimeout(()=>emit(ev),60)))}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.HCTerritorialDailyRhythmV1={version:1,state,current,dailyPulse,normalizeType,emit,storageKey:KEY};
})();