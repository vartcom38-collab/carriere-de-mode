/* Haute Couture Live — couche cartographique Gard.
   ...
   géocodage progressif et mis en cache ; les lieux fictifs utilisent des zones cohérentes sans fausse adresse exacte.
*/
(function(){
'use strict';
...
const iconFor=p=>/museum|culture|library|archive/.test(p.category)?'✦':/market|vintage|shop|mode/.test(p.category)?'✂':/nature|garden|landscape/.test(p.category)?'❧':/view|tower/.test(p.category)?'◌':/hotel/.test(p.category)?'⌂':/transport|station/.test(p.category)?'↗':/showroom|prestige/.test(p.category)?'◇':/people|cafe|studio|artisan/.test(p.category)?'♡':'♜';
...
function addMarker(layer,place,pt){const where=place.real===false?zoneLabel(place):(READY.has(place.id)?'Lieu de gameplay disponible':'Lieu préparé du Gard');const m=L.marker([pt.lat,pt.lng],{icon:markerIcon(place)}).bindTooltip(`<b>${esc(place.name)}</b><br>${esc(where)}`).on('click',()=>openPlace(place));m.addTo(layer);return m}
function run(map,p){const layer=L.layerGroup().addTo(map),cache=readCache(),city=norm(currentCity()),all=(p.places||[]).filter(x=>!SKIP.has(x.id)&&x.id!=='nimes-photo-studio');const priority=[...all].sort((a,b)=>{const ac=norm(a.city)===city?0:1,bc=norm(b.city)===city?0:1;if(ac!==bc)return ac-bc;return (READY.has(b.id)?1:0)-(READY.has(a.id)?1:0)});const base=centerOf(map),done=new Set();
  priority.filter(x=>x.real===false&&norm(x.city)===city).forEach(place=>{addMarker(layer,place,fictionalPoint(base,place));done.add(place.id)});
