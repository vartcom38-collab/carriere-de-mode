...
async function pageForPresence(code,dept,city,lat,lng){
 ...
 try{
   await page.waitForFunction(()=>!!window.HCTerritoryContext&&!!window.HCLocalMap&&!!window.HCTerritorialPlaceInterfaceV1,{timeout:12000});
 }catch(e){
   const diag=await page.evaluate(()=>({
     href:location.href,hasLeaflet:!!window.L,hasContext:!!window.HCTerritoryContext,hasMap:!!window.HCLocalMap,hasMedia:!!window.HCTerritorialPlaceMediaAURA,hasInterface:!!window.HCTerritorialPlaceInterfaceV1,
     presence:localStorage.getItem('haute-couture-current-presence-v1'),scripts:[...document.scripts].map(s=>s.src||'[inline]').filter(Boolean)
   })).catch(()=>({evaluation:'failed'}));
   await page.close();
   throw new Error(`${code} ${city}: bootstrap timeout · ${JSON.stringify(diag)} · pageErrors=${errors.join(' || ')} · consoleErrors=${consoleErrors.join(' || ')}`);
 }
...
async function testAin(){
 ...
 try{
   ...
   try{await page.waitForFunction(()=>{const i=document.querySelector('#tpHero img');return !!i&&i.complete&&i.naturalWidth>0},{timeout:15000})}catch(_){failures.push('Ain Brou: vraie image non chargée dans Chromium')}
   ...
   await page.locator('[data-tpa]').first().click();await page.waitForTimeout(200);
   ...
   const focusResult=await page.evaluate(()=>{
     ...
     window.HCFranceGeo=window.HCFranceGeo||{};window.HCFranceGeo.state={region:{nom:'Auvergne-Rhône-Alpes'},department:{code:'69',nom:'Rhône'},commune:{nom:'Lyon',lat:45.764,lng:4.835}};
   ...
   console.log('✓ AIN: photo réelle + source + fiction + temps + mémoire + anti-téléportation');
...
try{
 ...
 for(const [code,dept,city,lat,lng] of cases){
   ...
   try{
     const before=await page.evaluate(()=>localStorage.getItem('haute-couture-current-presence-v1'));
     await page.evaluate(({code,dept,city,lat,lng})=>window.HCTerritorialPlaceInterfaceV1.open({id:'qa-'+code,name:'QA '+city,dept:code,departmentName:dept,city,lat,lng,cat:'culture',text:'QA interface',fictional:true}),{code,dept,city,lat,lng});
     await page.locator('#hcTerritorialPlaceOverlay.open').waitFor({timeout:5000});
     const after=await page.evaluate(()=>localStorage.getItem('haute-couture-current-presence-v1'));
     if(before!==after)failures.push(`${code} ${city}: ouverture interface a modifié la présence`);
     if(errors.some(x=>/SyntaxError|ReferenceError/.test(x)))failures.push(`${code} ${city}: ${errors.join(' | ')}`);
     console.log('✓ département',code,city,'bootstrap + interface');
   }finally{await page.close()}
 }
