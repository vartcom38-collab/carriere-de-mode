...
const SUPPORTED=new Set(['fabric','craft','culture','heritage','markets','vintage','nature','view','libraries','jewelry','people','cafe','shop','photo']);
...
out.summary={
 totalMarkers:out.markers.length,realMarkers:real.length,fictionalMarkers:fictional.length,
 realWithImageAndSource:real.filter(x=>x.hasImage&&x.hasSource).length,
 realMissingValidatedMedia:missingRealMedia.length,
 fictionalWithImage:fictional.filter(x=>x.hasImage).length,
...
out.realMissingValidatedMedia=missingRealMedia;
out.unsupported=unsupported;
out.noCategory=noCategory;
out.noText=noText;
out.noUnlock=noUnlock;

const reportPath=path.join(HC,'qa','territories-ui-media-audit-report.json');
...
if(missingRealMedia.length){
 console.log('\n--- LIEUX RÉELS SANS IMAGE + SOURCE VALIDÉES ---');
 for(const m of missingRealMedia)console.log(`${m.dept||'?'} | ${m.city||'?'} | ${m.id} | ${m.name||'?'} | ${m.file}`);
}
if(unsupported.length||noCategory.length){
 console.error('\nINTERFACE CONTRACT FAILURE: catégorie non couverte ou absente.');
 for(const m of [...unsupported,...noCategory])console.error(`${m.id} | cat=${m.cat||'ABSENTE'} | ${m.file}`);
 process.exit(2);
}
console.log('\nAudit terminé. Les médias manquants sont bloquants pour la validation régionale, mais restent des constats tant que leur corpus n’est pas complété.');
