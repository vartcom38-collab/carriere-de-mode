...
await page.route('**/*',r=>{
 ...
 if(['image','media','font'].includes(t)||u.includes('tile.openstreetmap.org'))return r.abort();
 return r.continue();
});
await page.addInitScript(()=>{
 window.__qaCapturedPlaces=[];
 let value;
 Object.defineProperty(window,'HCLocalMap',{configurable:true,get(){return value},set(v){
  value=v;
  if(v?.addMarker&&!v.__qaWrapped){
...
const entries=Object.entries(DEPTS).filter(([c])=>!ONLY||c===ONLY);
...
for(const [code,cities] of entries){
 for(const city of cities){
  ...
  const onPage=e=>pageErrors.push(String(e.message||e));
  ...
  try{
   ...
   for(let i=0;i<12&&stable<3;i++){
    ...
    if(n>0&&n===previous)stable++;else if(n!==previous){stable=0;previous=n}
   ...
   for(const p of unique){
    ...
    if(fictional){
     if(!/FICTIONNEL/.test(ui.status)||!/ILLUSTRATION|FICTIF/.test(ui.flag))failures.push(tag+' · fiction/événement évolutif mal signalé');
    }else{
...
console.log(`\nTOTAL CLIQUABLES UNIQUES${ONLY?' '+ONLY:''}: ${rows.length}`);
console.log(`RÉELS: ${rows.filter(x=>!x.fictional).length} · FICTIFS/ÉVOLUTIFS: ${rows.filter(x=>x.fictional).length}`);
