...
function unlock(target,id,title,extra={}){if(!active())return null;const s=read(),key=target+':'+id;if(!s.discoveries[key]){s.discoveries[key]={id,title,target,city:currentCity(),day:day(),year:year(),...extra};write(s);window.dispatchEvent(new CustomEvent('hc-territorial-unlock',{detail:{departmentCode:DEPT,target,discovery:s.discoveries[key]}}))}return s.discoveries[key]}
...
window.HCAllierTerritorialGameplay={version:3,people,briefs,secrets,eventFamilies,experiences,meet,personRecord,eventEdition,syncEvents,observeReference,useExperience,temporaryObjects,state:read,visit,unlock,active};
