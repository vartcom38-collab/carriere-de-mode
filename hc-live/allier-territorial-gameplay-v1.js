/* Haute Couture Live — Allier territorial gameplay V1
...
(function(){
'use strict';
if(window.__HCAllierTerritorialGameplayV1)return;window.__HCAllierTerritorialGameplayV1=true;
...
const write=s=>{localStorage.setItem(KEY,JSON.stringify(s));window.dispatchEvent(new CustomEvent('hc-allier-territorial-state',{detail:s}));return s};
...
const people=[
 {id:'allier-p-moulins-costume',city:'Moulins',name:'Élise Marceau',role:'costumière de scène',systems:['Atelier','Book','scène']},
 {id:'allier-p-moulins-doc',city:'Moulins',name:'Nora Besset',role:'documentaliste costume',systems:['archives','Book','recherche']},
 {id:'allier-p-vichy-event',city:'Vichy',name:'Camille Vernet',role:'chargée d’événementiel',systems:['clientes','réception','agenda']},
 {id:'allier-p-vichy-photo',city:'Vichy',name:'Sacha Morel',role:'photographe indépendant',systems:['Book','shooting','réseau']},
 {id:'allier-p-montlucon-scene',city:'Montluçon',name:'Mina Perrin',role:'habilleuse de scène',systems:['costume','musique','réseau']},
 {id:'allier-p-bourbon-client',city:'Bourbon-l’Archambault',name:'Claire Derval',role:'cliente de cure récurrente',systems:['clientes','cérémonie','mémoire']}
];
const briefs=[
 ...
 {id:'allier-b-vichy-02',city:'Vichy',title:'Silhouette photo entre galerie et parc',systems:['Book','shooting'],needs:2},
...
const secrets=[
 ...
 {id:'allier-s-montlucon-tournee',city:'Montluçon',title:'Une équipe de passage cherche une solution costume',needs:3}
];
const eventFamilies=[
 {id:'allier-e-moulins-scene',city:'Moulins',label:'Moulins entre en scène',months:[6,7,8],themes:['costume et lumière','patrimoine en mouvement','ville nocturne','scène et silhouette']},
 {id:'allier-e-vichy-napoleon',city:'Vichy',label:'Fête historique de Vichy',months:[4,5,6],themes:['élégance historique relue','réception et costume','architecture et silhouette','parure et promenade']},
...
function meet(id,action='hello'){const p=people.find(x=>x.id===id);if(!p)return null;const s=read(),r=s.people[id]||{id,city:p.city,name:p.name,role:p.role,met:false,professionalOpen:false,interactions:0,history:[]};r.met=true;r.interactions=(r.interactions||0)+1;r.lastDay=day();if(action==='work'&&r.interactions>=2)r.professionalOpen=true;r.history.push({day:day(),action});r.history=r.history.slice(-30);s.people[id]=r;write(s);if(r.interactions===2)emit('phone_lead',{id:'contact-'+id,city:p.city,personId:id,title:'Un contact de '+p.city,text:p.name+' t’a reconnue et garde ton travail en tête.'});unlockForCity(p.city);return r}
...
window.HCAllierTerritorialGameplay={version:1,people,briefs,secrets,eventFamilies,meet,personRecord,eventEdition,syncEvents,observeReference,state:read};
syncEvents();
})();