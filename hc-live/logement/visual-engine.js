...
(function(){
  ...
  function showImage(main,url){main.style.background='#efe4d9';main.innerHTML=`<img src="${url}" alt="Illustration du logement" style="width:100%;height:100%;object-fit:cover;display:block">`}
  function showPending(main,v,msg){main.style.background='linear-gradient(135deg,#fff4ea,#ead5c2 46%,#c9d8c7)';main.innerHTML=`<div style="max-width:520px;padding:18px;background:#fffaf1e8;border:1px solid #d9bfae;border-radius:16px;font:14px/1.5 Georgia,serif;color:#493a33"><b style="display:block;font:900 10px Arial,sans-serif;letter-spacing:.12em;margin-bottom:7px">${msg}</b><strong style="font-size:21px">${v.archetypeLabel}</strong><br>${v.architecture} · ${v.decorSignature}<br>${v.lightSignature}<br><small style="display:block;margin-top:8px;color:#7e685d">${v.visualSeed}</small></div>`}
  }
  async function generateView(x,view='main'){
    ...
    if(view==='main')showPending(main,v,'GÉNÉRATION DU VISUEL…');
    try{const out=await window.HCVisualService.request(x,contextFor(x),view);if(out.url&&view==='main')showImage(main,out.url);else if(out.pending&&view==='main')showPending(main,v,'MOTEUR VISUEL PRÊT — CONNEXION API À FINALISER')}catch(e){if(view==='main')showPending(main,v,'VISUEL EN ATTENTE — SERVICE INDISPONIBLE')}
  }
  function paintDetail(x){if(!x||!window.HCVisualDNA)return;const v=window.HCVisualDNA.hydrate(x,contextFor(x)),main=document.getElementById('mainVisual');if(main){main.dataset.visualSeed=v.visualSeed;main.dataset.visualStatus=v.visualStatus;if(v.assets&&v.assets.mainImage)showImage(main,v.assets.mainImage);else showPending(main,v,'VISUEL UNIQUE PRÉPARÉ')}
    const thumbs=document.querySelectorAll('.thumb');const names=['Pièce principale','Cuisine','Salle d’eau','Extérieur / vue'];const keys=['main','kitchen','bathroom','window'];thumbs.forEach((t,i)=>{t.dataset.visualSeed=v.visualSeed;t.dataset.promptKey=keys[i];t.style.cursor='pointer';const sp=t.querySelector('span');if(sp)sp.textContent=names[i];t.onclick=async()=>{const asset=keys[i]==='main'?v.assets.mainImage:v.assets.gallery[keys[i]];if(asset){showImage(main,asset);return}showPending(main,v,'GÉNÉRATION DE LA VUE…');try{const out=await window.HCVisualService.request(x,contextFor(x),keys[i]);if(out.url)showImage(main,out.url)}catch(e){showPending(main,v,'VUE EN ATTENTE')}}});
    ...
    x.visual=v;window.HCVisualDNA.save(x);generateView(x,'main');
  }
