...
function important(){const s=state(),g=game(),p=phone();const msg=(g.messages||[]).find(m=>m&&!m.read&&!s.seen['m:'+m.id]&&(m.action||/recommand|opportun|cliente|rendez-vous|projet|collab|agence|atelier/i.test((m.subject||'')+' '+(m.text||''))));if(msg)return{kind:'message',id:'m:'+msg.id,data:msg};const opp=(p.opportunities||[]).find(o=>o&&o.status==='offered'&&!s.seen['o:'+o.id]);if(opp)return{kind:'opportunity',id:'o:'+opp.id,data:opp};return null}
...
window.HCPhoneImportantDialogueBridgeV1={version:1,important,scene,maybeOpen,state,storageKey:KEY};
})();
