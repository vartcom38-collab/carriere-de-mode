...
function capture(){
  ...
  return {schema:SNAPSHOT_SCHEMA,snapshotVersion:1,capturedAt:now(),page:location.pathname,storage,skipped};
}
function validServerSnapshot(data){return !!(data&&data.schema===SNAPSHOT_SCHEMA&&data.storage&&typeof data.storage==='object')}
function snapshotHasPlayableState(snapshot){
  if(!validServerSnapshot(snapshot))return false;
  const s=snapshot.storage||{};
  return !!(s['haute-couture-game-state-v1']||s['haute-couture-start-path-v1']||s['haute-couture-home']||s['haute-couture-residence']||s['haute-couture-school-choice-v1']);
}
function hasLocalPlayableState(){
  return !!(localStorage.getItem('haute-couture-game-state-v1')||localStorage.getItem('haute-couture-start-path-v1')||localStorage.getItem('haute-couture-home')||localStorage.getItem('haute-couture-residence')||localStorage.getItem('haute-couture-school-choice-v1'));
}
function isFreshStartScreen(){
  const p=location.pathname.replace(/\/+$/,'');
  const current=localStorage.getItem('haute-couture-current-screen');
  return current==='start-choice'||/\/start-choice$/i.test(p);
}
...
async function saveNow(reason){
  ...
  const data=capture();
  try{
    const response=await fetch(SAVE_URL,{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({slot:SLOT,playerName:playerName(),gameVersion:'hc-live-server-bridge-v1',data})});
    ...
    if(reason==='new-game-replace-server'){
...
function schedule(reason,delay=1200){clearTimeout(timer);timer=setTimeout(()=>saveNow(reason),delay)}
async function boot(){
  if(booted)return;
  booted=true;
  ...
  // Une nouvelle partie ne restaure JAMAIS l'ancien serveur et ne recharge JAMAIS la page.
  // Le marqueur reste présent jusqu'à ce que le snapshot neuf ait été écrit avec succès.
  if(resetRequested||isFreshStartScreen()){
    try{sessionStorage.removeItem(RESTORE_GUARD)}catch(e){}
    schedule('new-game-replace-server',250);
    return;
  }
...
window.addEventListener('pagehide',()=>{if(navigator.sendBeacon){try{const payload=JSON.stringify({slot:SLOT,playerName:playerName(),gameVersion:'hc-live-server-bridge-v1',data:capture()});navigator.sendBeacon(SAVE_URL,new Blob([payload],{type:'application/json'}))}catch(e){}}});
