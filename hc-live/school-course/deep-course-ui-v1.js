...
function practiceRoute(id,d){
 if(d?.stylusRoute)return d.stylusRoute;
 if(id==='w1-drawing')return '../school-drawing/';
 if(id.includes('drawing'))return '../school-drawing-level3/';
 if(id==='w1-pattern'||id.includes('pattern'))return '../school-pattern-transformations/';
 if(id==='w1-drape'||id.includes('drape'))return '../school-draping-intro/';
 if(id==='w1-textile'||id.includes('textile'))return '../school-textile-lab/';
 return null;
}
function routeLabel(id){if(id.includes('drawing'))return'OUVRIR L’ATELIER DESSIN';if(id.includes('pattern'))return'OUVRIR L’ATELIER MODÉLISME';if(id.includes('drape'))return'OUVRIR L’ATELIER MOULAGE';if(id.includes('textile'))return'OUVRIR L’ATELIER MATIÈRE';return'OUVRIR LA PRATIQUE GUIDÉE'}
function render(){
