/* Haute Couture Live — sélecteur dynamique des banques ville V2
   Choisit personnages / briefs / secrets / événements sans répétition immédiate.
*/
(function(){'use strict';if(window.HCCityContentSelectorV2)return;window.HCCityContentSelectorV2=true;
...
function edition(city){const b=bankFor(city);if(!b)return null;const g=game(),y=new Date(g?.clock?.iso||Date.now()).getFullYear(),arr=b.events||[];if(!arr.length)return null;const h=history(city),candidates=arr.filter(x=>!h.events.slice(-3).includes(`${x.id}:${y}`));const item=choose(candidates.length?candidates:arr,[city,y,season(),h.events.length].join('|'));if(!item)return null;const editionId=`${item.id}:${y}`;if(!h.events.includes(editionId)){h.events.push(editionId);h.events=h.events.slice(-30);const s=read();s.cities[city]=h;write(s)}return{...item,year:y,season:season(),editionId,fictionalFuture:y>2026}}
window.HCCityContentSelectorV1=window.HCCityContentSelectorV2={version:2,bankFor,pickBrief,pickPerson,revealSecret,edition,season,level,history};window.dispatchEvent(new CustomEvent('hc-city-content-selector-ready'));
})();