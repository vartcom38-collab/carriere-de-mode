...
#characters .hc-focus.hc-focus-ines .hc-focus-portrait-final img{object-fit:cover!important;object-position:50% 18%!important;transform:scale(1.55)!important;mix-blend-mode:normal!important;filter:none!important}
...
function syncFocus(){const root=$('#characters .hc-cast');if(!root)return;const focused=$('.hc-person.focused',root)?.dataset.id;const f=$('.hc-focus',root);if(!f)return;f.classList.toggle('hc-focus-ines',focused==='ines');if(focused!=='ines')return;const h=$('.hc-focus-header-final',f);if(h){const img=$('.hc-focus-portrait-final img',h);if(img&&window.HC_INES_FULL_V2){img.src=window.HC_INES_FULL_V2;img.alt='Inès'}const title=$('.hc-focus-title-final h3',h);if(title)title.textContent='Inès';const vibe=$('.hc-focus-title-final .vibe',h);if(vibe)vibe.textContent='Solaire · couleur & mouvement'}}
function apply(){css();const a=mountCard();syncFocus();return a}
function boot(){let n=0;const tick=()=>{if(apply()||++n>=25)return;setTimeout(tick,100)};tick();document.addEventListener('click',e=>{if(e.target.closest?.('#characters .hc-person,#characters .hc-focus-nav'))setTimeout(apply,0)},true)}
