(function(){
  ...
  function render(){const c=current(),map=mapEl();if(!c||!map)return false;css();const rec=touchVisit()||{visits:1};let layer=$('.hc-personal-map-layer',map);if(layer)layer.remove();layer=document.createElement('div');layer.className='hc-personal-map-layer';const entries=placeEntries(map,c);const stage=Math.min(3,1+Math.floor(((rec.visits||1)-1)/2)+Math.floor(entries.length/4));map.classList.remove('hc-map-stage-1','hc-map-stage-2','hc-map-stage-3');map.classList.add('hc-map-stage-'+stage);
    entries.forEach((e,i)=>{const n=document.createElement('div');n.className='hc-personal-ink kind-'+e.kind;n.style.left=e.x+'%';n.style.top=e.y+'%';n.style.setProperty('--r',(((hash(c.code+'|'+e.id)%7)-3)*.7)+'deg');n.textContent=e.label;layer.appendChild(n)});map.appendChild(layer);
  ...
  function boot(){css();let last='';setInterval(()=>{const c=current(),map=mapEl();if(!c||!map)return;const signature=c.code+'|'+(game()?.world?.day||1)+'|'+map.childElementCount+'|'+localStorage.length;if(signature!==last){last=signature;render()}},450);const loc=$('#location');if(loc)new MutationObserver(()=>setTimeout(render,90)).observe(loc,{subtree:true,childList:true,attributes:true});window.addEventListener('storage',()=>setTimeout(render,50));}
