(function(){
  ...
  function localEntries(c){const r=load()[c.code],life=Object.values(r?.localLife||{}),out=[];life.filter(x=>x.known).slice(-4).forEach((x,i)=>out.push({id:'local-'+x.id,label:(x.met?'● ':'○ ')+x.place+(x.met?' · '+x.person:''),x:66+(i%2)*9,y:66-Math.floor(i/2)*10,kind:x.met?'person':'local'}));return out}
  ...
  function css(){if($('#hcPersonalMapStyles'))return;const st=document.createElement('style');st.id='hcPersonalMapStyles';st.textContent=`
    .hc-personal-map-layer{position:absolute;inset:0;z-index:9;pointer-events:none}.hc-personal-ink{position:absolute;transform:translate(-50%,-50%) rotate(var(--r));max-width:150px;padding:2px 5px;background:rgba(255,250,239,.74);border-bottom:1px solid rgba(111,143,107,.34);font:10px/1.15 Georgia,serif;color:#596257;box-shadow:0 1px 0 rgba(255,255,255,.5);opacity:.88}.hc-personal-ink.kind-home{border-bottom-color:rgba(212,130,90,.5);font-style:italic}.hc-personal-ink.kind-memory{background:rgba(255,247,223,.78);font-style:italic}.hc-personal-ink.kind-note,.hc-personal-ink.kind-local{border:1px dashed rgba(111,143,107,.3);transform:translate(-50%,-50%) rotate(-2deg)}.hc-personal-ink.kind-person{background:rgba(232,239,226,.88);border-bottom-color:rgba(83,112,76,.45)}.hc-personal-stamp{position:absolute;left:4%;bottom:4%;z-index:17;pointer-events:none;background:rgba(255,250,239,.9);border:1px solid rgba(89,97,78,.22);padding:5px 8px;transform:rotate(-1deg);font:9px Georgia,serif;color:#667060}.hc-personal-stamp b{font-weight:400;font-size:11px}.hc-personal-stamp small{display:block;font:8px Arial,sans-serif;letter-spacing:.09em;text-transform:uppercase;color:#8a8579}.hc-map-stage-1 .hc-personal-ink:nth-child(n+3){display:none}.hc-map-stage-2 .hc-personal-ink:nth-child(n+5){display:none}@media(max-width:850px){.hc-personal-ink{font-size:9px;max-width:118px;padding:2px 4px}.hc-personal-stamp{left:3%;bottom:3%}}
  ...
  function loadLocalLife(){if(window.__HCAinLocalLifeInstalled||window.__HCAinLocalLifeLoading)return;window.__HCAinLocalLifeLoading=true;const s=document.createElement('script');s.src='./ain-local-life-engine.js?v=20260909-local1';s.onload=()=>{window.__HCAinLocalLifeLoading=false};document.head.appendChild(s)}
  function boot(){css();loadLocalLife();let last='';const tick=()=>{const c=current(),map=mapEl();if(!c||!map||!document.querySelector('#location.active'))return;const r=load()[c.code]||{},signature=c.code+'|'+(game()?.world?.day||1)+'|'+(r.visits||0)+'|'+Object.keys(r.localLife||{}).length+'|'+localStorage.length;if(signature!==last){last=signature;render()}};setInterval(tick,1200);window.addEventListener('storage',()=>{last='';setTimeout(tick,80)});window.addEventListener('hc-ain-local-memory-changed',()=>{last='';setTimeout(tick,80)});document.addEventListener('click',e=>{if(e.target.closest('#choiceList,.crumbs,.region-pin,.hc-acm-hotspot,.hc-bourg-hotspot,.hc-ain-local-life')){last='';setTimeout(tick,120)}})}
  window.HCAinPersonalMapMemory={load,current,touchVisit,render};
