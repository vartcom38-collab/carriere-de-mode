...
const DEPT_BY_CITY={
 ...
 'Clermont-Ferrand':'63','Thiers':'63','Riom':'63','Volvic':'63','Le Mont-Dore':'63','La Bourboule':'63','Lyon':'69','Villefranche-sur-Saône':'69','Tarare':'69','Villeurbanne':'69','Nîmes':'30'
};
...
function normalizePlace(input,meta={}){if(!input)return null;const department=input.department&&typeof input.department==='object'?input.department:null;const commune=input.commune&&typeof input.commune==='object'?input.commune:null;const city=input.city||input.communeName||commune?.nom||input.name||department?.nom||null;const code=deptCode(input.departmentCode??input.deptCode??department?.code??input.departmentName??(typeof input.department==='string'?input.department:null))||DEPT_BY_CITY[city]||null;return {id:input.id||commune?.code||null,city,commune:commune||null,department:department||input.department||null,departmentCode:code,departmentName:input.departmentName||department?.nom||(typeof input.department==='string'?input.department:null)||null,region:input.region||input.regionName||null,lat:num(input.lat??commune?.lat??department?.lat),lng:num(input.lng??input.lon??commune?.lng??commune?.lon??department?.lng??department?.lon),source:meta.source||input.source||null,reason:meta.reason||input.reason||null,enteredAt:input.enteredAt||null};}
function getStoredPresence(){return normalizePlace(read(PRESENCE_KEY),{source:'presence'});}
function getTravelPresence(){const g=read(GAME_KEY),w=g?.world;if(!w?.currentCity&&!w?.currentDepartment)return null;return normalizePlace({city:w.currentCity,department:w.currentDepartment,departmentName:w.currentDepartment,departmentCode:w.currentDepartmentCode,region:w.currentRegion,lat:w.currentLat,lng:w.currentLng},{source:'travel-world',reason:'current-location'});}
function getPresence(){return getStoredPresence()||getTravelPresence();}
function syncGameWorld(p){const g=read(GAME_KEY);if(!g)return;g.world=g.world||{};if(p.city)g.world.currentCity=p.city;if(p.departmentName||p.department)g.world.currentDepartment=p.departmentName||(typeof p.department==='string'?p.department:null);if(p.departmentCode)g.world.currentDepartmentCode=p.departmentCode;if(p.region)g.world.currentRegion=p.region;if(p.lat!=null)g.world.currentLat=p.lat;if(p.lng!=null)g.world.currentLng=p.lng;try{if(window.HauteCoutureCore?.save)window.HauteCoutureCore.save(g);else localStorage.setItem(GAME_KEY,JSON.stringify(g))}catch(_){localStorage.setItem(GAME_KEY,JSON.stringify(g))}}
function setPresence(place,reason='visit'){const p=normalizePlace(place,{source:'presence',reason});if(!p)return null;p.enteredAt=new Date().toISOString();localStorage.setItem(PRESENCE_KEY,JSON.stringify(p));syncGameWorld(p);window.dispatchEvent(new CustomEvent('hc-territory-presence',{detail:p}));loadDepartmentGameplay(p);return p;}
...
const CITY_UNIVERSE_PACK={
 'Lyon':'rhone-lyon-city-universe-v2.js?v=20260910-lyondense2',
 'Villeurbanne':'villeurbanne-city-universe-v2.js?v=20260910-villeurbannedense1',
 'Clermont-Ferrand':'clermont-ferrand-city-universe-v3.js?v=20260910-clermontdense3',
 'Saint-Étienne':'saint-etienne-city-universe-v3.js?v=20260910-stetiennedense3',
 'Moulins':'moulins-vichy-city-universe-v3.js?v=20260910-mvdense2','Vichy':'moulins-vichy-city-universe-v3.js?v=20260910-mvdense2',
 'Aurillac':'aurillac-city-universe-v3.js?v=20260910-aurillacdense2',
 'Le Puy-en-Velay':'le-puy-en-velay-city-universe-v3.js?v=20260910-lepuydense2',
 'Bourg-en-Bresse':'bourg-oyonnax-city-universe-v3.js?v=20260910-bourgdense2','Oyonnax':'bourg-oyonnax-city-universe-v3.js?v=20260910-oyonnaxdense2'
};
...
function loadDepartmentGameplay(place=getPresence()){
 ...
 const city=String(place.city||''),citySrc=CITY_UNIVERSE_PACK[city],bankSrc=CITY_CONTENT_BANK[city];
 if(citySrc)loadScript('hcCityUniverse'+city.replace(/\W+/g,''),citySrc);
 if(bankSrc)loadScript('hcCityContentBank'+city.replace(/\W+/g,''),bankSrc);
}
