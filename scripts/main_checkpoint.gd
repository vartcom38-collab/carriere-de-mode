...
func _ready() -> void:
    atelier.texture = load("res://assets/atelier_checkpoint.svg") as Texture2D
    ...
    warm_light.color = Color(1.0, 0.86, 0.58, 0.0)
    _make_dust()
    _connect_menu()
...
func _process(delta: float) -> void:
    ...
    if not cinematic_running:
        ...
        warm_light.color.a = 0.018 + (sin(idle_time * 0.38) + 1.0) * 0.011
    _animate_dust(delta)
...
func _finish_intro() -> void:
    ...
    create_tween().tween_property(menu_hitboxes, "modulate:a", 1.0, 0.65)
...
func _make_dust() -> void:
    for _i in range(46):
        var p := ColorRect.new()
        var s := randf_range(1.0, 2.6)
        p.size = Vector2(s, s)
        p.position = Vector2(randf_range(520.0, 1510.0), randf_range(180.0, 820.0))
        p.color = Color(1.0, 0.94, 0.72, randf_range(0.06, 0.22))
