...
func _draw_clouds(w: float, h: float) -> void:
    ...
    for cloud in clouds:
        var x := (float(cloud["x"]) + fmod(elapsed * float(cloud["speed"]), 0.12)) * w
        if x > 0.78 * w:
            x = 0.50 * w + (x - 0.78 * w)
        var y := float(cloud["y"]) * h
        var s := float(cloud["scale"])
        var c := Color(1.0, 1.0, 1.0, float(cloud["alpha"]))
...
func _draw_smoke(w: float, h: float) -> void:
    var chimneys := [Vector2(0.795 * w, 0.225 * h), Vector2(0.825 * w, 0.23 * h)]

    for i in range(chimneys.size()):
        var root := chimneys[i]
        for j in range(8):
            var age := fmod(elapsed * (0.45 + i * 0.03) + j * 0.18, 1.55)
            var x := root.x + sin(elapsed * 0.85 + j * 0.72 + i) * (2.0 + age * 8.0)
            var y := root.y - age * 54.0
            var radius := 3.0 + age * 6.5
            var alpha := max(0.015, 0.075 - age * 0.038)
...
func _draw_birds(w: float, h: float) -> void:
    ...
    for bird in birds:
        var x := (float(bird["x"]) + fmod(elapsed * float(bird["speed"]), 0.22)) * w
        if x > 0.76 * w:
            x = 0.50 * w + (x - 0.76 * w)
        var y := float(bird["y"]) * h + sin(elapsed * 0.58 + float(bird["phase"])) * 3.2
        var s := float(bird["scale"])
        var flap := sin(elapsed * 4.0 + float(bird["phase"])) * 2.6 * s
        var wing := 7.0 * s
        var middle := Vector2(x, y - 2.0 * s)
        var color := Color(0.25, 0.18, 0.13, 0.50)
